...
if __name__ == "__main__":
    ...
    def resample_audio(batch):
        ...
        batch["audio"] = [librosa.load(io.BytesIO(audio["audio"]["bytes"]), sr=44100) for audio in batch["audio"]]
    ...
    os.makedirs("/home/fbsdev009/bucket/dac_dataset_2",exist_ok=True)
        ...
        if jax.process_index() == 0:
            if i%10240 == 0:
                ...
                if writer is not None:
                    writer.close() 
                writer = ArrayRecordWriter(f"/home/fbsdev009/bucket/dac_dataset_2/mls_eng_train_part_{num}.arrayrecord", 'group_size:1')
